...
class TradingViewTradovateBroker:
    ...
    def place_order(
        ...
        current_price = self.price_provider.get_price(order.instrument)

        side = order.position.lower()
        target_price = order.price
        if side == "buy":
            if target_price < current_price:
                order_type = OrderTypes.LIMIT.value  # Buy BELOW market
            else:
                order_type = OrderTypes.STOP.value   # Buy ABOVE market
        else:  # sell
            if target_price > current_price:
                order_type = OrderTypes.LIMIT.value  # Sell ABOVE market
            else:
                order_type = OrderTypes.STOP.value   # Sell BELOW market


        if order_type == OrderTypes.LIMIT.value:
            price_type = 'limitPrice'
        elif order_type == OrderTypes.STOP.value:
