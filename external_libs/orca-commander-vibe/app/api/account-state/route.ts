...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`https://tv-demo.tradovateapi.com/accounts/${accountId}/state?locale=en`, {
      method: 'GET',
      headers: {
    ...
    if (stateData.s === 'ok' && stateData.d) {
      // Extract the first 5 elements from amData for the financial metrics
      const amData = stateData.d.amData?.[0]?.[0] || [];
      const [totalPL, openPL, netLiq, totalMarginUsed, availableMargin] = amData;
      
      return NextResponse.json({
        success: true,
        data: {
          balance: stateData.d.balance,
          unrealizedPl: stateData.d.unrealizedPl,
          equity: stateData.d.equity,
          totalPL: parseFloat(totalPL) || 0,
          openPL: parseFloat(openPL) || 0,
          netLiq: parseFloat(netLiq) || 0,
          totalMarginUsed: parseFloat(totalMarginUsed) || 0,
    ...
    return NextResponse.json({
      success: false,
      error: 'Invalid response format from Tradovate API',
      details: stateData
    }, { status: 500 });
