  ...
  try {
    ...
    if (!accountName || !orderId) {
      return NextResponse.json({ 
        success: false, 
        error: 'Account and order ID parameters are required' 
      }, { status: 400 });
    }
    ...
    const response = await fetch(`https://tv-demo.tradovateapi.com/accounts/${accountId}/orders/${orderId}?locale=en`, {
      method: 'DELETE',
      headers: {
    ...
    if (responseData.s === 'error') {
      return NextResponse.json({
        success: false,
        error: responseData.errmsg || 'Order cancellation failed',
        details: responseData
      }, { status: 400 });
    }
