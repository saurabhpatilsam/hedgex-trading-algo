...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use the shared Redis client (same one used by pricing cards)
    const redisClient = await getSharedRedisClient();
    if (!redisClient) {
      return NextResponse.json({ error: 'Redis connection failed' }, { status: 500 });
    }

    // Fetch JWT token from Redis (similar to Python implementation)
    ...
    // No need to disconnect - using shared client

    // Make API call to Tradovate
    const response = await fetch(`https://tv-demo.tradovateapi.com/accounts/${accountId}/orders?locale=en`, {
      method: 'GET',
      headers: {
    ...
    const orders = ordersData.d || [];
    
    return NextResponse.json({
      success: true,
      orders: orders, // Use real API data
      accountId,
      accountName,
